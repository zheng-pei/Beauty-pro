 ...
 * @param string $tablekey 索引字段
 * @param string $param 参数值
 * @param string $posfield 目标字段获取
 * @return void
 * @author zhangyang
 */
function getMyValue($table,$tablekey,$param,$posfield){
    $model = D($table);
    $args = $model->where($tablekey."=".$param)->field($posfield)->select();
    echo $args[0][$posfield];
}
