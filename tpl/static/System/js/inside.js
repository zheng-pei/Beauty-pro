...
var G = {
    ...
    data: {},
    domain: {
        w: 'http://localhost/~yang/yuehaihotel/',
        t: 'http://localhost/~yang/yuehaihotel/',
        k: 'http://localhost/~yang/yuehaihotel/wm-xin-a'
    },
    set: {
        KindEditor_seting: {
            themeType: "simple",
    ...
    tip: function (data) {
        if (data.errno == 0) {
            if (data.tip) {
				...
				if(data.modal==1){
					G.ui.tips.suc(data.error, "close")
				}else{
...
G.logic.page = {
    ...
    common: function () {
        ...
        $(".datempick").length > 0 && $(".datempick").datepicker('setStartDate', new Date(Date.parse(new Date())).toLocaleString().replace(/年|月/g, "-").replace(/日/g, " ").substr(0,10));
...
G.logic.uploadify = {
    ...
    set: function () {
        ...
        _self.op.el.uploadify({
			...
			//	alert(G.logic.uploadify.Callback(i.image));
